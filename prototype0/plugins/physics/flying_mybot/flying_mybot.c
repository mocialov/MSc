...
typedef enum { TAKING_OFF, SEARCH_OBSTACLE, MOVE_UP, AVOID_OBSTACLE,
    MOVE_DOWN, LANDING
} states;
...
{  
    ...
    /* We create a Ray object in the general space of ODE. */
    ray_geom = dCreateRay(s, RAY_LENGTH);
...
void webots_physics_step()
{
    ...
    int size;
    float *sensors = dWebotsReceive(&size);
    ...
    case LANDING:
        ...
        if (pause_counter == 0) {
            ...
            inversing_direction_counter =
                (int) (6 * ((float) rand() / RAND_MAX) + 1);
            turning_direction = (turning_direction == LEFT) ? RIGHT : LEFT;

            /* We send the new datas to the controller. */
            int buffer[2];
    ...
    dGeomRaySet(ray_geom, position[0], position[1], position[2], -rotation[2],
...
int webots_physics_collide(dGeomID g1, dGeomID g2)
{
    ...
    if (dAreGeomsSame(g1, ray_geom) || dAreGeomsSame(g2, ray_geom)) {
         ...
         */
        if (dSpaceQuery((dSpaceID) robot_geom, g1) == 1
            || dSpaceQuery((dSpaceID) robot_geom, g2) == 1) {
            return 1;
        }
        ...
        ray_distance = sqrt(d[0] * d[0] + d[1] * d[1] + d[2] * d[2]);

        dWebotsConsolePrintf
            ("Obstacle detected at a distance of %g after %g s\n", ray_distance,
             dWebotsGetTime() / 1000);

        ray_collision_occured = 2;
        ...
        if ((dAreGeomsSame(g1, floor_geom) && dSpaceQuery((dSpaceID) robot_geom, g2) == 1)
            || (dSpaceQuery((dSpaceID) robot_geom, g1) == 1
                && dAreGeomsSame(g2, floor_geom))) {
        n = dCollide(g1, g2, 10, &contact[0].geom, sizeof(dContact));
        if (n == 0) {
            return 1;
        }
         ...
         */
        for (i = 0; i < n; i++) {
            contact[i].surface.mode = dContactMu2 | dContactBounce |
                dContactApprox1 | dContactSoftCFM;
            ...
            dJointAttach(dJointCreateContact(world, contact_joint_group,
                                             &contact[i]), robot_body, NULL);
...
/*
 * This function is called every time ODE redraws the Scene.
 * This is the good place to draw OpenGL objects in the Scene.
 */
void webots_physics_draw(int pass, const char *view)
{
    ...
    /* 
     * Here we draw the sensor ray to make it visible. We also change its
     * color when it is detecting an obstacle.
     */
    if (ray_collision_occured == 0) {
    ...
    glVertex3d(start[0] + dir[0] * RAY_LENGTH,
               start[1] + dir[1] * RAY_LENGTH,
               start[2] + dir[2] * RAY_LENGTH);
    glEnd();

    /* Restore OpenGL state to the default values used by Webots */
